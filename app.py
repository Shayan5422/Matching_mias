            ...
            try:
                if col_key == 'delta_days':
                    ...
                    if pd.notna(comp_num):
                     ...
                     if pd.notna(comp_num):
                          entree_vals = entree_numeric_cols[col_key]
                          if col_key == 'annee_naiss':
                              # Apply 2-year tolerance for birth year
                              lower_bound = comp_num - 2
                              upper_bound = comp_num + 2
                              current_filter &= (entree_vals >= lower_bound) & (entree_vals <= upper_bound)
                          else:
                              # Ensure exact match for other numeric fields
...
if st.session_state.entree_data is not None and st.session_state.comparison_data is not None:
    ...
    if st.session_state.auto_matches is not None and not st.session_state.auto_matches.empty:
        ...
        *(La tolérance de ±2 jours est appliquée pour la durée de séjour et de ±2 ans pour l'année de naissance)*.
        """)
    ...
    if search_record_to_use is not None:
        ...
        for col_key, col_label in ALLOWED_COLUMNS.items():
             ...
             if col_key not in search_record_to_use.index:
                 ...
                 elif col_key == 'annee_naiss' and not is_search_val_missing: display_val = f"{int(search_val)} (±2 ans)"
            ...
            with st.spinner(f"Recherche des correspondances dans '{entree_filename}'..."):
                ...
                for filter_key, is_active in active_filters.items():
                    ...
                    if is_active and filter_key in search_record_to_use.index:
                        ...
                        # Add tolerance info for specific fields
                        if filter_key == 'annee_naiss' and not is_search_nan:
                            match_criteria_used.append(f"{ALLOWED_COLUMNS[filter_key]} = '{search_display}' (±2 ans)")
                        elif filter_key == 'delta_days' and not is_search_nan:
                            match_criteria_used.append(f"{ALLOWED_COLUMNS[filter_key]} = '{search_display}' (±2 jours)")
                        else:
                            ...
                            if filter_key == 'sexe':
                                ...
                                if is_search_nan:
                                     ...
                                     if pd.notna(search_num):
                                          ...
                                          results_df = results_df[entree_annee_col.notna() &
                                                              (entree_annee_col >= lower_bound) &
                                                              (entree_annee_col <= upper_bound)]
